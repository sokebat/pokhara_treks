...
export async function listRegions(): Promise<RegionRow[]> {
  return db.select().from(regions).orderBy(asc(regions.sortOrder), asc(regions.title));
}
...
export async function findRegionBySlug(slug: string): Promise<RegionRow | null> {
  const [row] = await db
    .select()
    .from(regions)
